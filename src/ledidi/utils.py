...
"""def with_new_sequence(element: dict, onehot_seq: torch.Tensor) -> dict:
    ...
    e["sequence"] = onehot_seq.to(device)         
    return e
"""
...
def apply_edits_to_sequence(element: dict,
                             ...
                             row_map: dict[str, int] = None, device="cuda") -> dict:
    """
...
def plot_multi_site_attributions(model: torch.nn.Module,
    ...
    """
    # Apply all edits
    updated = apply_edits_to_sequence(element, edits, device=device)
    ...
    plt.tight_layout()
    plt.savefig(f"attributions{str(edits[0])}.svg", format="svg", bbox_inches="tight")
    plt.show()
...
def make_intra_loop_mask(elem, device="cpu"):
    """
    Return BoolTensor (L,) where True = LOCK (can't edit).
    Only the middle is editable - between boundaries.
    """
    L = 524288
