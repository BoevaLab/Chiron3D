...
from src.ledidi.running_utils import process_ctcf_bed_peak, make_ctcf_edit_mask, col_to_base, plot_all_for_sequence, process_spklf_bed_peak, make_custom_edit_mask
...
def evaluate_element(elem, core_model, device, stripe, low, high, l_weight, max_iter, early_stop, tau, lr, run_dir):
    ...
    # get the peaks
    sp_klf_dict = process_spklf_bed_peak()
    input_mask = make_custom_edit_mask(elem, custom_dict=sp_klf_dict, flank=10, device=device)
...
def main():
    ...
    parser.add_argument("--low", type=float, default=0.8, help="Lower bound of acceptable ratio")
    parser.add_argument("--high", type=float, default=1.25, help="Upper bound of acceptable ratio")
    ...
    for idx in range(len(dataset)):
        elem = dataset[idx]
        stripe = elem.get("enrich_status_sum", "").strip()
