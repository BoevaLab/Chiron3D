...
class SmallEncoder(nn.Module):
    def __init__(self, in_channel, output_size=128, filter_size=5, num_blocks=11):
        super(SmallEncoder, self).__init__()
        self.filter_size = filter_size
        self.conv_start = nn.Sequential(
            nn.Conv1d(in_channel, 16, 3, 2, 1),
            nn.BatchNorm1d(16),
            nn.ReLU(),
        )
...
# Our adaptation
class EncoderSplitSmall(nn.Module):
    def __init__(self, num_epi, output_size=128, filter_size=5, num_blocks=11):
        super(EncoderSplitSmall, self).__init__()
        self.filter_size = filter_size
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + pe # something is wrong here - fix
...
# Our adaptation
class AttnModuleSmall(nn.Module):
    def __init__(self, hidden=128, layers=4, record_attn=False):
        super(AttnModuleSmall, self).__init__()
