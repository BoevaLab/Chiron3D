...
def main():
    ...
    for smooth in ["No", "50", "75", "100"]:
        if smooth == "No":
            cool_path = args.cool_file
        else:
            cool_path = f"/cluster/work/boeva/shoenig/ews-ml/data/A673_general/smoothed_hic/smooth_{smooth}_hic_chr19.cool"
        ds = GenomicDataset(
            regions_file_path=args.regions_file,
            cool_file_path=cool_path, #f"/cluster/work/boeva/shoenig/ews-ml/data/A673_general/smoothed_hic/smooth_75_hic_chr19.cool", #args.cool_file,
        ...
        for batch in tqdm(dl, total=len(dl)):
            ...
            for out, true in zip(output, batch["matrix"]):
                ...
                r_p, r_s, preds, targs = insulation_corr(out, true)
                l_mse = mse(out, true)
                ...
                insu_score_preds.append(preds)
                insu_score_targs.append(targs)


        dist_p_mat = np.asarray(dist_strat_pearson_list, dtype=float)  
        dist_s_mat = np.asarray(dist_strat_spearman_list, dtype=float)
        
        insu_score_preds = np.asarray(insu_score_preds)
        insu_score_targs = np.asarray(insu_score_targs)

        np.savez_compressed(
            os.path.join(args.out_dir, f"{smooth}_metrics_chr19.npz"),
            ...
            diag_x=xs,
            diag_y=ys,
            preds=insu_score_preds,
            targs=insu_score_targs
        )
