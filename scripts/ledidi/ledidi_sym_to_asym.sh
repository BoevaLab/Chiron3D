...
#SBATCH --output=/cluster/work/boeva/shoenig/ews-ml/RESULTS_SYM_TO_ASYM/log1.txt
...
python -u ledidi_sym_to_asym.py \
        --ckpt "${CKPT}" \
        --csv_out "${CSV_OUT}" \
        --device cuda \
        --run_dir /cluster/work/boeva/shoenig/ews-ml/RESULTS_SYM_TO_ASYM
echo "------------------------------------------------------------"
